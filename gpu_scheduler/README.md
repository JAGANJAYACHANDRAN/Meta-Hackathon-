...
| **Idle GPU cost** | − | `idle_gpus × hours × rate × hourly_rate × scale`. Rate = 0.20 (queue non-empty) or 0.05 (empty) |
| **Preemption burn** | − | `gpu_count × min(elapsed, 2h) × hourly_rate × 0.3 × scale` |
| **SLA violation** | − | Initial: `3.0×` one-time. Continuing: `0.5×` per step overdue |
| **Queue delay** | − | P0=0.50, P1=0.25 factor; P2/P3 = no penalty |
